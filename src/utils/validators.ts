...
export const validateCountry = (country: Partial<ICountry>): IValidationError | null => {
  const errors: IValidationError = {};

  if (!country.name) errors['name'] = 'is required';

  if (country.population === undefined || typeof country.population !== 'number' || country.population <= 0)
    errors['population'] = 'must be a positive number';

  if (country.currency_code === undefined || country.currency_code === null || country.currency_code === '')
...
export const errorHandler = (err: IAppError, req: Request, res: Response, next: NextFunction) => {
  ...
  if (status >= 500 && status !== 503) {
    console.error(`[${status}] ${err.stack ?? err.message}`);
  }
